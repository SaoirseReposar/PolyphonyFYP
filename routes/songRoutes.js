const express = require('express');
const router = express.Router();
const db = require('../database');
const translationService = require('../services/Translationservice');
const spotifyService = require('../services/spotifyService');
const lrclibService = require('../services/lrclibService');
const { classifySong } = require('../services/classificationService');
const axios = require('axios');


router.get('/songs', async (req, res) => {
    try {
        const result = await db.query(
            'SELECT * FROM songs ORDER BY created_at DESC'
        );
        res.json({ success: true, songs: result.rows });
    } catch (error) {
        console.error('Error fetching songs:', error);
        res.status(500).json({ success: false, error: 'Failed to fetch songs' });
    }
});


router.get('/songs/:id', async (req, res) => {
    try {
        const { id } = req.params;
        
        const songResult = await db.query(
            'SELECT * FROM songs WHERE id = $1',
            [id]
        );
        
        if (songResult.rows.length === 0) {
            return res.status(404).json({ success: false, error: 'Song not found' });
        }

        if (req.session && req.session.user) {
            db.query(
                `INSERT INTO user_song_progress (user_id, song_id, last_accessed, completed)
                 VALUES ($1, $2, NOW(), true)
                 ON CONFLICT (user_id, song_id) DO UPDATE SET last_accessed = NOW(), completed = true`,
                [req.session.user.user_id, id]
            ).catch(err => console.error('Progress tracking error:', err));
        }
        
        const lyricsResult = await db.query(
            'SELECT * FROM lyrics WHERE song_id = $1 ORDER BY line_number',
            [id]
        );
        
        const song = songResult.rows[0];
        song.lyrics = lyricsResult.rows;
        
        res.json({ success: true, song });
    } catch (error) {
        console.error('Error fetching song:', error);
        res.status(500).json({ success: false, error: 'Failed to fetch song' });
    }
});


router.get('/songs/spotify/:spotifyTrackId', async (req, res) => {
    try {
        const { spotifyTrackId } = req.params;
        
        const songResult = await db.query(
            `SELECT s.*, 
                    (SELECT COUNT(*) FROM lyrics WHERE song_id = s.id) as lyrics_count
             FROM songs s 
             WHERE s.spotify_track_id = $1`,
            [spotifyTrackId]
        );
        
        if (songResult.rows.length === 0) {
            return res.json({ success: false, message: 'Song not found' });
        }
        
        const song = songResult.rows[0];
        
        res.json({
            success: true,
            song: {
                id: song.id,
                title: song.title,
                artist: song.artist,
                has_lyrics: parseInt(song.lyrics_count) > 0
            }
        });
    } catch (error) {
        console.error('Error checking song:', error);
        res.status(500).json({ success: false, error: 'Failed to check song' });
    }
});



router.post('/songs', async (req, res) => {
    try {
        const { title, artist, language, difficulty, spotify_track_id } = req.body;
        
        let spotifyData = null;
        if (spotify_track_id) {
            spotifyData = await spotifyService.getTrackInfo(spotify_track_id);
        }
        
        const result = await db.query(
            `INSERT INTO songs (title, artist, language, difficulty, spotify_track_id, duration_ms, album_art_url)
             VALUES ($1, $2, $3, $4, $5, $6, $7)
             RETURNING *`,
            [
                title,
                artist,
                language,
                difficulty,
                spotify_track_id,
                spotifyData?.duration_ms || null,
                spotifyData?.album_art || null
            ]
        );
        
        res.json({ success: true, song: result.rows[0] });
    } catch (error) {
        console.error('Error creating song:', error);
        res.status(500).json({ success: false, error: 'Failed to create song' });
    }
});


router.post('/songs/import', async (req, res) => {
    const { spotifyTrackId, trackName, artistName, albumArtUrl, durationMs, availableMarkets = [], spotifyToken } = req.body;

    if (!spotifyTrackId || !trackName || !artistName) {
        return res.status(400).json({ success: false, error: 'Missing required fields' });
    }

    console.log(`\n🎵 Auto-importing "${trackName}" by ${artistName}...`);

    try {
        const existing = await db.query('SELECT id FROM songs WHERE spotify_track_id = $1', [spotifyTrackId]);
        if (existing.rows.length > 0) {
            return res.json({ success: true, songId: existing.rows[0].id, alreadyExists: true });
        }

        const lyricsData = await lrclibService.getSyncedLyrics(trackName, artistName);
        if (!lyricsData || !lyricsData.syncedLyrics) {
            return res.status(422).json({ success: false, error: 'no_lyrics', message: `No synced lyrics found for "${trackName}" on LRCLIB.` });
        }

        const parsedLyrics = lrclibService.parseLRC(lyricsData.syncedLyrics);

        let audioFeatures = null;
        if (spotifyToken) {
            try {
                const afRes = await axios.get(`https://api.spotify.com/v1/audio-features/${spotifyTrackId}`, {
                    headers: { Authorization: `Bearer ${spotifyToken}` }
                });
                audioFeatures = afRes.data;
            } catch (e) { /* non-fatal */ }
        }

        const classification = classifySong({
            plainLyrics: lyricsData.plainLyrics || parsedLyrics.map(l => l.text).join('\n'),
            parsedLyrics,
            audioFeatures,
            availableMarkets
        });

        if (!classification.language || !['spanish','french','german','korean','japanese','portuguese','italian'].includes(classification.language)) {
            return res.status(422).json({ success: false, error: 'unsupported_language', detectedLanguage: classification.language });
        }

        const songResult = await db.query(
            `INSERT INTO songs (spotify_track_id, title, artist, language, difficulty, duration_ms, album_art_url, tags, difficulty_score)
             VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9)
             ON CONFLICT (spotify_track_id) DO UPDATE SET title=EXCLUDED.title, updated_at=CURRENT_TIMESTAMP
             RETURNING id`,
            [spotifyTrackId, trackName, artistName, classification.language, classification.difficulty,
             durationMs || (lyricsData.duration * 1000) || 180000, albumArtUrl || null,
             classification.mood || null, classification.difficultyScore]
        );

        const songId = songResult.rows[0].id;
        const textsToTranslate = parsedLyrics.map(l => l.text);
        let translations;
        try {
            translations = await translationService.translateBatch(textsToTranslate, classification.language, 'EN-US');
        } catch (e) {
            translations = textsToTranslate.map(() => null);
        }

        for (let i = 0; i < parsedLyrics.length; i++) {
            await db.query(
                `INSERT INTO lyrics (song_id, line_number, timestamp_ms, original_text, translated_text, language_to)
                 VALUES ($1,$2,$3,$4,$5,$6)
                 ON CONFLICT (song_id, line_number) DO UPDATE SET
                    timestamp_ms=EXCLUDED.timestamp_ms, original_text=EXCLUDED.original_text, translated_text=EXCLUDED.translated_text`,
                [songId, parsedLyrics[i].line, Math.round(parsedLyrics[i].time * 1000), parsedLyrics[i].text, translations[i] || null, 'en']
            );
        }

        console.log(` Import complete: "${trackName}" (id: ${songId})`);
        res.json({ success: true, songId, classification });

    } catch (error) {
        console.error(' Import error:', error);
        res.status(500).json({ success: false, error: 'import_failed', message: error.message });
    }
});

router.post('/songs/:id/lyrics', async (req, res) => {
    try {
        const { id } = req.params;
        const { lyrics } = req.body; 
        
        const songResult = await db.query('SELECT language FROM songs WHERE id = $1', [id]);
        if (songResult.rows.length === 0) {
            return res.status(404).json({ success: false, error: 'Song not found' });
        }
        
        const sourceLang = songResult.rows[0].language;
        
        const textsToTranslate = lyrics.map(l => l.text);
        
        console.log(`Translating ${textsToTranslate.length} lines from ${sourceLang} to English...`);
        const translations = await translationService.translateBatch(textsToTranslate, sourceLang, 'EN-US');
        
        const insertedLyrics = [];
        for (let i = 0; i < lyrics.length; i++) {
            const lyric = lyrics[i];
            const translation = translations[i];
            
            const result = await db.query(
                `INSERT INTO lyrics (song_id, line_number, timestamp_ms, original_text, translated_text, language_to)
                 VALUES ($1, $2, $3, $4, $5, $6)
                 ON CONFLICT (song_id, line_number) 
                 DO UPDATE SET 
                    timestamp_ms = EXCLUDED.timestamp_ms,
                    original_text = EXCLUDED.original_text,
                    translated_text = EXCLUDED.translated_text
                 RETURNING *`,
                [id, lyric.line_number, lyric.timestamp_ms, lyric.text, translation, 'en']
            );
            
            insertedLyrics.push(result.rows[0]);
        }
        
        res.json({ 
            success: true, 
            message: `${insertedLyrics.length} lyrics added and translated`,
            lyrics: insertedLyrics 
        });
    } catch (error) {
        console.error('Error adding lyrics:', error);
        res.status(500).json({ success: false, error: 'Failed to add lyrics', details: error.message });
    }
});


router.post('/translate/word', async (req, res) => {
    try {
        const { word, language, sentence } = req.body;
        
        const cleanWord = word.toLowerCase().replace(/[.,!?;¿¡]/g, '').trim();
        
        const cacheResult = await db.query(
            `SELECT * FROM word_translations 
             WHERE word = $1 AND source_language = $2 AND target_language = 'en'`,
            [cleanWord, language]
        );
        
        if (cacheResult.rows.length > 0) {
            await db.query(
                'UPDATE word_translations SET usage_count = usage_count + 1 WHERE id = $1',
                [cacheResult.rows[0].id]
            );
            
            return res.json({ 
                success: true, 
                translation: cacheResult.rows[0],
                cached: true 
            });
        }
        
        const translationData = await translationService.translateWord(
            cleanWord, 
            language, 
            sentence,
            'EN-US'
        );
        

        const insertResult = await db.query(
            `INSERT INTO word_translations (word, source_language, target_language, translation, context)
             VALUES ($1, $2, $3, $4, $5)
             RETURNING *`,
            [cleanWord, language, 'en', translationData.translation, translationData.context]
        );
        
        res.json({ 
            success: true, 
            translation: insertResult.rows[0],
            cached: false 
        });
    } catch (error) {
        console.error('Error translating word:', error);
        res.status(500).json({ success: false, error: 'Failed to translate word' });
    }
});


router.post('/words/save', async (req, res) => {
    try {
        const { user_id, word_id, song_id } = req.body;
        
        const result = await db.query(
            `INSERT INTO user_saved_words (user_id, word_id, song_id)
             VALUES ($1, $2, $3)
             ON CONFLICT DO NOTHING
             RETURNING *`,
            [user_id || 1, word_id, song_id] 
        );
        
        res.json({ success: true, saved_word: result.rows[0] });
    } catch (error) {
        console.error('Error saving word:', error);
        res.status(500).json({ success: false, error: 'Failed to save word' });
    }
});


router.get('/words/saved', async (req, res) => {
    try {
        const { user_id } = req.query;
        
        const result = await db.query(
            `SELECT 
                usw.id,
                wt.word,
                wt.translation,
                wt.source_language,
                s.title as song_title,
                usw.saved_at
             FROM user_saved_words usw
             JOIN word_translations wt ON usw.word_id = wt.id
             LEFT JOIN songs s ON usw.song_id = s.id
             WHERE usw.user_id = $1
             ORDER BY usw.saved_at DESC`,
            [user_id || 1]
        );
        
        res.json({ success: true, words: result.rows });
    } catch (error) {
        console.error('Error fetching saved words:', error);
        res.status(500).json({ success: false, error: 'Failed to fetch saved words' });
    }
});


router.get('/spotify/track/:trackId', async (req, res) => {
    try {
        const { trackId } = req.params;
        const trackInfo = await spotifyService.getTrackInfo(trackId);
        res.json({ success: true, track: trackInfo });
    } catch (error) {
        console.error('Error fetching Spotify track:', error);
        res.status(500).json({ success: false, error: 'Failed to fetch track info' });
    }
});


router.get('/translation/usage', async (req, res) => {
    try {
        const usage = await translationService.getUsage();
        res.json({ success: true, usage });
    } catch (error) {
        console.error('Error fetching usage:', error);
        res.status(500).json({ success: false, error: 'Failed to fetch usage' });
    }
});

module.exports = router;